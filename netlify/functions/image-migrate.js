...
// Original Supabase image URLs keyed by title
const IMAGE_URLS = {
...
exports.handler = async () => {
  ...
  try {
    // Find recipe with no image_data and no valid image_url
    const dbResult = await tursoRun(
      "SELECT id, title FROM recipes WHERE (image_data IS NULL OR image_data = '') AND title != 'Air Fryer Chips' LIMIT 1"
    );
    ...
    const srcUrl = IMAGE_URLS[recipe.title];

    if (!srcUrl) {
      // No image for this recipe, mark as done by setting empty string
      await tursoRun("UPDATE recipes SET image_data = 'none' WHERE id = ?", [{ type: 'text', value: String(recipe.id) }]);
    ...
    if (!imgRes.ok) throw new Error(`Fetch failed ${imgRes.status} for ${recipe.title}`);
    ...
    await tursoRun(
      'UPDATE recipes SET image_data = ? WHERE id = ?',
      [{ type: 'text', value: dataUrl }, { type: 'text', value: String(recipe.id) }]
    );

    const remaining = await tursoRun(
      "SELECT COUNT(*) FROM recipes WHERE (image_data IS NULL OR image_data = '') AND title != 'Air Fryer Chips'"
    );
